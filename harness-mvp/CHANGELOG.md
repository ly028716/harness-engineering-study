...
- **任务依赖可视化**: 新增 `harness plan graph` 命令，生成 Mermaid 格式依赖图
  - 按任务状态着色（DONE 绿/WIP 蓝/TODO 灰/BLOCKED 红）
  - 循环依赖检测（DFS 白灰黑标记法），红色高亮环节点
  - 关键路径分析（最长 Required 依赖链）
  - 拓扑排序批次（可并行执行的任务分组）
  - 文本分析报告模式（`--output report`）
...
- 新增 32 个依赖可视化测试用例（test_dependency_graph.py）
- 测试套件从 355 扩展至 429 个（+74 个，+21%）
